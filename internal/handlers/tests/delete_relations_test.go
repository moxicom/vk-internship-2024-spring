...
func TestHandler_DeleteRelation(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			mockBehavior: func(s *mock_service.MockRelations, input models.RelationMoviesActors) {
				s.EXPECT().DeleteRelation(input).Return(nil).AnyTimes()
			},
		...
		},
		{
			...
			input:     models.RelationMoviesActors{},
			mockBehavior: func(s *mock_service.MockRelations, input models.RelationMoviesActors) {
				s.EXPECT().DeleteRelation(input).Return(errors.New("empty request body")).AnyTimes()
			},
